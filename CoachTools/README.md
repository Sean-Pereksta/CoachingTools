...
**Clean Upload** is authoritative manual placement: recognized filename families select the dock regardless of headers or reporting dates; unidentified readable files offer a dock selector whose answer is final. Each successful save atomically replaces that dock’s previous data and adds it to the persistent Update Data set without dropping other established docks. Update Data keeps its existing matching and processing behavior, replaying each dock’s saved people selection. Removing a dock removes it from that set. Data Settings shows **People: All** or an expandable list of selected names. If an unusual file cannot apply the selected people filter, Clean Upload accepts all rows and explicitly reports that coverage alongside the requested names. Unreadable files and storage failures still report errors without deleting committed data.
