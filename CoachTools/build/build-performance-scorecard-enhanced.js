...
source = requiredReplace(
  source,
  /function metricPack\(person,metricId,cohort\)\{[^\n]*\}/,
  ...
  'scope metric pool calculation'
);
source = requiredReplace(
  source,
  /function rowData\(rep,cohort\)\{[^\n]*\}/,
  ...
  'pooled scorecard row data'
);
source = requiredReplace(
  source,
  /function getRows\(\)\{[^\n]*\}/,
  "function getRows(){const reps=scopedReps(),pools=buildMetricPools(reps),rows=reps.map(r=>rowData(r,reps,pools)),k=state.sort.key,d=state.sort.dir;if(k==='rank-overall'||k==='rank-score'||String(k).startsWith('rank:'))return rows;rows.sort((a,b)=>{let av=sortValue(a,k),bv=sortValue(b,k);if(typeof av==='string')return av.localeCompare(String(bv))*d;if(!Number.isFinite(av))av=d>0?Infinity:-Infinity;if(!Number.isFinite(bv))bv=d>0?Infinity:-Infinity;return(av-bv)*d});return rows}",
