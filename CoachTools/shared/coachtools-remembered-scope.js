(function attachCoachToolsRememberedScope(root) {
  ...
  function makeButton(label, action, className) {
    ...
    button.type = 'button';
    button.className = className || 'command-button secondary';
  ...
  function addCleanUploadButtons() {
    ...
    if (topUpdate) topUpdate.insertAdjacentElement('afterend', makeButton('Clean Upload', 'clean-upload-data'));

    const panelUpdate = root.document.querySelector('#dataPanel [data-action="update-data"]');
    if (panelUpdate) panelUpdate.insertAdjacentElement('afterend', makeButton('Clean Upload', 'clean-upload-data'));

    const menuUpdate = root.document.querySelector('#startMenu [data-action="update-data"]');
    if (menuUpdate) {
      const button = root.document.createElement('button');
      button.type = 'button';
      ...
      strong.textContent = 'Clean Upload';
      const small = root.document.createElement('small');
      small.textContent = 'Choose the data files again and save the current scope as the baseline for future updates';
      copy.append(strong, small);
      button.append(iconWrap, copy);
      menuUpdate.insertAdjacentElement('afterend', button);
    }
  }
