...
function attachModelReferencePicker(root=document){
  // Eligibility only. One delegated controller owns columns, metrics and models.
  root.querySelectorAll('[data-header-list],[data-research-expression-input],textarea[data-pb="expression"],input[data-rf="expression"]').forEach(inp=>{
    inp.dataset.modelPickerAttached='1'; inp.removeAttribute('list'); inp.setAttribute('autocomplete','off');
  });
}
...
function renderResearchColumnsEditor(){
  ...
  els.researchColumns.innerHTML=(state.editingResearchColumns||[]).map((c,i)=>{
    ...
    const fieldControl=expr
      ...
      : `<div class="field"><label>${within?(dateWithin?'Date field/expression':'Value field/expression'):(isConv?'Display column':'Field/Expression')}</label><div class="researchInputGear"><input data-rc="field" data-i="${i}" data-header-list="researchHeaderSuggestions" value="${esc(c.field||'')}">${researchGearButton('columnField:'+i)}</div></div>`;
...
function renderPercentBuilderEditor(item={}){
  ...
  els.researchPercentBuilder.innerHTML=`<div class="researchRuleSection"><strong>Percent Builder</strong><div class="hint">Show the % of a cohort that matches a rule, out of a selected denominator, grouped by the Research group / X axis.</div><div class="researchStepGrid">
    ...
    ${custom?`<div class="field" style="grid-column:1/-1"><label>where custom numeric expression</label><textarea data-pb="expression" rows="3" data-header-list="researchHeaderSuggestions" placeholder="sum(![Non-Date Database].[cash apps]) / sum(![Non-Date Database].[cash opps])">${esc(pb.expression||'')}</textarea>${expressionDiagnosticHtml(pb.expression,item.source||'', 'display')}</div>`:`<div class="field"><label>where field</label><input data-pb-rule="field" data-header-list="researchHeaderSuggestions" value="${esc(rule.field||'')}" placeholder="documented.coaching.description"></div>`}
