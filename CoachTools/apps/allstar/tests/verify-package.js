...
check('Qualtrics concern history is report-specific and remains portable',['concernOccurrenceKey','classifyConcernHistory','summarizeConcernReportCounts','concernHistoryReportCount','ambiguous name-only legacy rows','Appearance History','Summary','All-Star Representative Key','coachMatch(rec,rule,reportDate,3)','Review Option</strong> = repeated concern despite related coaching'].every(x=>(generator+'\n'+read('qualtrics/insights.js')+'\n'+read('js/qualtrics-bridge.js')).includes(x))&&portable.includes('qualtrics_concern_history_'));
check('Documented Coaching direct counts are person-first and diagnose unusable dates',read('js/calculations.js').includes("source==='documented_coaching') return true")&&workflow.includes('documentedCoachingDiagnostics')&&workflow.includes('no usable coaching date column was detected'));
...
check('Allstar startup has one monotonic progress owner',models.includes('createAllStarStartupJob')&&models.includes('job.preventedRegressions++')&&app.includes('function startAllStar()')&&app.includes('runAllStarStartupPipeline'));
