...
check('Qualtrics concern history is deterministic, rule-specific, and portable',['concernOccurrenceKey','classifyConcernHistory','Appearance History','Summary','All-Star Representative Key','coachMatch(rec,rule,reportDate,3)','Review Option</strong> = repeated concern despite related coaching'].every(x=>(generator+'\n'+read('qualtrics/insights.js')+'\n'+read('js/qualtrics-bridge.js')).includes(x))&&portable.includes('qualtrics_concern_history_'));
