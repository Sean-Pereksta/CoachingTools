...
function metricFromRows(rows, type) {
  ...
  return {
    ...
    insurance: { num: 0, den: 0, value: NaN },
    commercial: { num: 0, den: 0, value: NaN },
...
function run() {
  const eightRows = Array.from({ length: 8 }, (_, index) => row('rep-one', 'Representative One', monday(index), index + 1, 20));
  ...
  for (let index = 0; index < 5; index += 1) {
    twoRepRows.push(row('rep-a', 'Representative A', monday(index), 5, 10));
    twoRepRows.push(row('rep-b', 'Representative B', monday(index), 7, 10));
  }
  ...
  const weighted = build([record('weighted', [row('rep-weighted', 'Weighted Rep', monday(0), 1, 2), row('rep-weighted', 'Weighted Rep', monday(1), 9, 100)])]);
  ...
  const original = record('original-week', [row('rep-corrected', 'Corrected Rep', monday(0), 2, 10)], { importedAt: '2026-06-08T00:00:00.000Z', supersededBy: 'corrected-week' });
  const corrected = record('corrected-week', [row('rep-corrected', 'Corrected Rep', monday(0), 8, 10)], { importedAt: '2026-06-09T00:00:00.000Z', version: 2, replacedDatasetId: 'original-week' });
  ...
  const overlapRecord = record('current-and-history', [row('rep-overlap', 'Overlap Rep', monday(0), 4, 10)]);
  ...
  const older = record('older-history', [row('rep-combined', 'Combined Rep', monday(0), 2, 10), row('rep-combined', 'Combined Rep', monday(1), 3, 10)], { importedAt: '2026-06-15T00:00:00.000Z' });
  const current = record('multi-current', [row('rep-combined', 'Combined Rep', monday(1), 7, 10), row('rep-combined', 'Combined Rep', monday(2), 5, 10), row('rep-combined', 'Combined Rep', monday(3), 6, 10)], { importedAt: '2026-07-01T00:00:00.000Z', version: 2 });
  const combined = build([older, current]);
  assert.strictEqual(combined.byPerson.get('rep-combined').length, 4);
  assert.strictEqual(combined.byPerson.get('rep-combined').find(point => point.week === monday(1)).consumer.num, 7, 'The newer overlapping source should win deterministically.');

  const missing = build([record('missing-week', [0, 1, 3, 4].map(index => row('rep-missing', 'Missing Week Rep', monday(index), 5, 10)))]);
  assert.deepStrictEqual(Array.from(missing.byPerson.get('rep-missing'), point => point.week), [0, 1, 3, 4].map(monday), 'A missing business week must not be fabricated.');

  const transfer = build([record('coach-transfer', [
    row('rep-transfer', 'Transfer Rep', monday(0), 4, 10, { Coach: 'Old Coach' }),
    row('rep-transfer', 'Transfer Rep', monday(1), 6, 10, { Coach: 'New Coach' })
  ...
  const duplicateNames = build([record('duplicate-names', [
    row('person-alpha', 'Same Display Name', monday(0), 2, 10, { Email: 'alpha@example.test' }),
    row('person-beta', 'Same Display Name', monday(0), 8, 10, { Email: 'beta@example.test' }),
    row('person-alpha', 'Same Display Name', monday(1), 3, 10, { Email: 'alpha@example.test' }),
    row('person-beta', 'Same Display Name', monday(1), 9, 10, { Email: 'beta@example.test' })
  ...
  const retailReferral = build(
    [record('retail-source', [row('rep-both', 'Both Sources Rep', monday(0), 4, 10, { 'Wiper Count': 5, 'Wiper Jobs': 10 })])],
    [record('referral-source', [row('rep-both', 'Both Sources Rep', monday(0), 7, 10, { 'Wipers Accepted': 2, 'Wipers Asked': 10 })])]
  );
  ...
  assert.strictEqual(weekly.weekStartKey('06/01/2026 - 06/07/2026'), '2026-06-01');
  assert.strictEqual(weekly.weekStartKey('2026-W23'), '2026-06-01');
